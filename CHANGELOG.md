...
- Added a new tool that generates package contained parent classes of server-side entities that fully reflect the interface from `*.def` files. Also the package has the API of the server modules KBEngine.py by the official doc and python types generated by the types.xml file. [See](https://github.com/ve-i-uj/enki#assetsapi)
...
- The "pyperclip" library moved to the release environment from the dev environment
- requirements.txt was updated
- Makefile is deleted

### Fixed

- Added the ".env" file contained the PYTHONPATH variable to fix "pipenv shell" dependencies
- Disabled attribute access in FixedDict class (recursion error on deep copy)
