...
class UsetTypeParser:
    ...
    def __init__(
        self, user_type_dir: Path, external_libs_path: Path | None = None
    ) -> None:
    ...
    def parse(self) -> UserTypeInfos:
        ...
        sys.path.append(
            str((self._user_type_dir.parent / "server_common").absolute())
        )
        ...
        for path in self._user_type_dir.rglob("*.py"):
            ...
            for attr_name in dir(module):
                ...
                if (
                    not hasattr(attr, "createObjFromDict")
                ...
                signs = {
                    "createObjFromDict": inspect.signature(
                        attr.createObjFromDict
                    ),
                ...
                for method_name, sign in signs.items():
                    if isinstance(attr, type):
                        ...
                        cls = attr
                        if (
                            isinstance(cls.__dict__.get(method_name), classmethod)
                            ...
                            isinstance(
                                cls.__dict__.get(method_name), staticmethod
                            )
                            and len(sign.parameters) == 1
                        ):
                            ...
                            and next(iter(sign.parameters.keys())) == "self"
                        ):
                ...
                if all(oks.values()):
                    converter_info = ConverterInfo(
                        name=attr_name, fd_type="Any", obj_type="Any"
                    )
                    for method_name, sign in signs.items():
                        ...
                        if method_name == "createObjFromDict":
                            type_or_name = next(
                                iter(sign.parameters.values())
                            ).annotation
                            if isinstance(type_or_name, str):
                                fd_type = type_or_name
                            elif (
                                hasattr(type_or_name, "_name")
                                and type_or_name._name == "Dict"
                            ):
                                fd_type = "Dict"
                            else:
                                fd_type = type_or_name.__name__
                            if (
                                converter_info.fd_type == "Any"
                                and fd_type != "_empty"
                            ):
                        ...
                        if (
                            converter_info.fd_type != "Any"
                            and converter_info.obj_type != "Any"
                        ):
                            break
                    ...
                    res[module.__name__][converter_info.name] = UserTypeInfo(
                        module_name=module.__name__, converter_info=converter_info
                    )
