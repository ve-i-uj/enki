...
from tools.parsers import EntitiesXMLData, EntitiesXMLParser, EntityDefParser
...
def _normalizy_entity_module(exml_data: EntitiesXMLData) -> None:
    ...
    for e_data in exml_data.get_all():
        ...
        if e_data.hasBase and not base_module_path.exists():
            ...
            logger.info(
                f"There is no module {_EModuleComp.BASE.name.lower()}/{e_data.name}. "
...
def main() -> None:
    ...
    updated_exml_data = parsers.utils.normalize_entitiesxml(exml_data, edef_data)
    updated_exml_data.write_to_file(settings.UPDATED_ENTITIES_XML_PATH)
    logger.info(
        f"The updated entities.xml file has been written to the "
