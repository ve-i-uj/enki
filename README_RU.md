...
Кластерная архитектура KBEngine подразумевает, что компонент Machine должен работать на каждом узле кластера. Также необходимо указать адреса всех существующих узлов в файле конфигурации «kbengine.xml» перед запуском компонента Machine ([см. здесь](https://github.com/kbengine/kbengine/discussions/1589)). В случае запуска кластера в контейнерах Docker нужно тогда создавать сеть Docker с фиксированными адресами под каждый контейнер и все эти адреса помещать в конфиг перед стартом, чтобы Machine в каждом компоненте знали о друг друге - это не удобно. Кроме того, официальная документация Docker [рекомендует](https://docs.docker.com/config/containers/multi-service_container/) избегать использования одного контейнера, отвечающего за несколько аспектов вашего приложения в целом.

Чтобы решить эту проблему, я переписал компонент Machine, повторив Machine API. Новый компонент называется Supervisor. Он регистрирует компоненты и отвечает на запросы адресов компонентов. Его поведение похоже на  Machine, но Supervisor только один на весь кластер, он работает в отдельном контейнере и может динамически регистрировать компоненты. [Supervisor](enki/app/supervisor) основан на библиотеке Python «Enki».
...
Т.е. для каждой сущности, интерфейса и компонента будет сгененированы полный API: методы, свойства, удалённые вызовы, типы параметров, определённые в файлах `entity_defs/*.def` и `types.xml` (включая типы, которые возвращают конвертеры, подключенные к FIXED_DICT).
...
* 1) игровой (игровые сущности, реднер, UI, игровая логика)
