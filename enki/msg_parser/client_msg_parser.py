...
class OnUpdateBaseDirMsgParser(IMsgParser):
    ...
    def parse(self, msg: Message) -> OnUpdateBaseDirMsgParserResult:
        ...
        vector, _data_tail = VECTOR3.decode(memoryview(data))
...
class InitSpaceDataParsedMsgData(ParsedMsgData):
    ...
    space_id: KBESpaceId
    pairs: dict[KBEString, KBEString]
...
class InitSpaceDataMsgParser(IMsgParser):
    ...
    def parse(self, msg: Message) -> InitSpaceDataMsgParserResult:
        ...
        while data:
            ...
            pairs[KBEString(key)] = KBEString(value)

        pd = InitSpaceDataParsedMsgData(KBESpaceId(space_id), pairs)
...
class SetSpaceDataMsgParserResult(MsgParserResult):
    ...
    result: SetSpaceDataParsedMsgData | None
...
class SetSpaceDataMsgParser(IMsgParser):
    ...
    def parse(self, msg: Message) -> SetSpaceDataMsgParserResult:
        ...
        logger.debug("[%s] (%s)", self, devonly.func_args_values())
        values: tuple[Any, ...] = msg.get_values()
        space_id = KBESpaceId(values[0])
        key = KBEString(values[1])
        value = KBEString(values[2])
...
class DelSpaceDataMsgParserResult(MsgParserResult):
    ...
    result: DelSpaceDataParsedMsgData | None
...
class OnStreamDataStartedParsedMsgData(ParsedMsgData):
    ...
    @property
    def stream_id(self) -> KBEStreamId:
        """Получить идентификатор потока.

        Returns:
            числовой идентификатор потока данных

        """
        return self.streamId

    @property
    def stream_size(self) -> KBEUInt32:
    ...
    @property
    def stream_descr(self) -> KBEString:
...
class OnStreamDataRecvParsedMsgData(ParsedMsgData):
    ...
    @property
    def stream_chunk(self) -> KBERowByteData:
        """Получить текущий чанк данных потока.

        Returns:
            Байты текущего чанка данных
...
class OnStreamDataCompletedMsgParserResult(MsgParserResult):
    ...
    result: OnStreamDataCompletedParsedMsgData | None
