...
# TODO: [2025-06-25 11:18 burov_alexey@mail.ru]:
# В коде используется только cellapp.entitydefAliasID .
# Ещё в примере переопределяется (видимо, чтобы настройки выставить). И ещё
# конфиг похоже в пример генерируется. Ещё в парсере зачем-то используется.
