...
class _RegisteredComponentsStorage:
    ...
    def register_component(self, comp_info: ComponentInfo) -> None:
        ...
        if comp_type in self._single_comp_info_by_type:
            ...
            if (
                old_pd is not None
                and old_pd.componentID != comp_info.componentID
            ):
                logger.info(
...
class Supervisor(IStartable, IServerMsgReceiver):
    ...
    def __init__(self, udp_addr: Addr, tcp_addr: Addr) -> None:
        ...
        udp_addr = Addr(
            server.get_real_host_ip(udp_addr.ip_addr), udp_addr.port
        )
        tcp_addr = Addr(
            server.get_real_host_ip(tcp_addr.ip_addr), tcp_addr.port
        )
        ...
        self._handlers: dict[int, _SupervisorHandler] = {
            ...
            msgspec.supervisor.onStopComponent.id: _OnStopComponentHandler(
                self
            ),
        }
...
class _QueryComponentIDHandler(_SupervisorHandler[UDPMsgBackChannel]):
    ...
    async def handle(
        ...
        client = UdpMsgClient(
            Addr(
                back_channel.conn_info.client_addr.ip_addr,
                Port(pd.callback_port),
            ),
...
class _OnFindInterfaceAddrHandler(_SupervisorHandler[UDPMsgBackChannel]):
    ...
    async def handle(
        ...
        for info in infos:
            ...
            onBroadcastInterface_msg = (
                Message.create(  # pylint: disable=invalid-name
