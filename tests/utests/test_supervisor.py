...
class ComponentStorageTestCase(unittest.TestCase):
    ...
    def test_only_machine(self):
        ...
        info = storage.get_single_component_info(ComponentType.MACHINE_TYPE)
        assert info is not None
        assert info.component_type == ComponentType.MACHINE_TYPE
    ...
    def test_register_logger(self):
        ...
        logger_info.componentType = ComponentType.LOGGER_TYPE.value
        logger_info.componentID = self._app.generate_component_id()
        storage.register_component(logger_info)

        info = storage.get_single_component_info(ComponentType.LOGGER_TYPE)
        assert info is not None
    ...
    def test_deregister_logger(self):
        ...
        storage.deregister_single_component(ComponentType.LOGGER_TYPE)
        assert storage.get_single_component_info(ComponentType.LOGGER_TYPE) is None
