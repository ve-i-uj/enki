...
from enki.handlers.server_handlers.baseapphandler import (
    OnAppActiveTickHandler,
    OnBroadcastGlobalDataChangedHandler,
    OnEntityAutoLoadCBFromDBMgrHandler,
    OnDbmgrInitCompletedHandlerResult,
    OnEntityGetCellHandler,
...
class OnDbmgrInitCompletedHandlerTestCase(TestCase):
    def test_onDbmgrInitCompleted(self):
        handlers = server_handlers.SERVER_HANDLERS.get("baseapp")
...
class OnGetEntityAppFromDbmgrTestCase(TestCase):
    def test_onRegisterNewApp(self):
        ...
        pd = res.result
        assert pd.componentID == 8001
        assert pd.internal_address == Addr(ip_addr="172.23.0.10", port=39959)
