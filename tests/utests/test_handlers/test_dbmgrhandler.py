...
from enki.handlers.server_handlers.dbmgrhandler import (
    EntityAutoLoadHandler,
    OnBroadcastGlobalDataChangedHandlerResult,
    SyncEntityStreamTemplateHandlerResult,
)
...
class OnBroadcastGlobalDataChangedHandlerTestCase(TestCase):
    def test_onBroadcastGlobalDataChanged(self):
        handlers = server_handlers.SERVER_HANDLERS.get("dbmgr")
        assert handlers is not None
        handler = handlers.get(msgspec.app.dbmgr.onBroadcastGlobalDataChanged.id)
        ...
        serializer = kbepickle.get_serializer_for(ComponentType.DBMGR)
...
class SyncEntityStreamTemplateTestCase(TestCase):
    def test_syncEntityStreamTemplate(self):
        ...
        handlers = server_handlers.SERVER_HANDLERS.get("dbmgr")
        assert handlers is not None
        handler = handlers.get(msgspec.app.dbmgr.syncEntityStreamTemplate.id)
        assert handler is not None
