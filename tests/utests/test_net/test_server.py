...
class TestUDPServer:
    ...
    async def test_server_received_data(self):
        ...
        class MyUDPServer(UDPServer):
            def on_receive_data(
                self, data: memoryview, addr: tuple[str, int]
            ) -> None:
                received_data.append(data)
        ...
        server_addr = Addr("0.0.0.0", Port(get_free_port()))
        server = MyUDPServer(
            server_addr,
        )
        res = await server.start()
...
class _UnderTestingTCPServer(TCPServer):
    ...
    def __init__(self, addr) -> None:
        ...
        self.call_data_of_on_end_receive_client_data: list[ConnInfo] = []
...
class TestTCPServer:
    ...
    async def test_on_receive_client_data(self):
        ...
        server_host, server_port = "0.0.0.0", Port(get_free_port())
    ...
    async def test_back_channel_send_data(self):
        ...
        server = _UnderTestingTCPServer(Addr(server_host, Port(server_port)))
        res = await server.start()
        assert res.success
