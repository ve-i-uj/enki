...
from enki.handlers.server_handlers.loginapphandler import (
    OnBaseappInitProgressHandler,
    OnDbmgrInitCompletedHandler,
    OnAppActiveTickHandler,
)
...
class OnDbmgrInitCompletedTestCase(TestCase):
    def test_onDbmgrInitCompleted(self):
        handlers = server_handlers.SERVER_HANDLERS.get("loginapp")
        assert handlers is not None
        handler_cls = handlers.get(msgspec.app.loginapp.onDbmgrInitCompleted.id)
        ...
        serializer = kbepickle.get_serializer_for(ComponentType.LOGINAPP)
...
class OnBaseappInitProgressTestCase(TestCase):
    def test_OnBaseappInitProgressHandler(self):
        handlers = server_handlers.SERVER_HANDLERS.get("loginapp")
        assert handlers is not None
        handler_cls = handlers.get(msgspec.app.loginapp.onBaseappInitProgress.id)
        assert handler_cls is not None
...
class OnAppActiveTickTestCase(TestCase):
    def test_OnAppActiveTickHandler(self):
        handlers = server_handlers.SERVER_HANDLERS.get("loginapp")
        assert handlers is not None
        handler_cls = handlers.get(msgspec.app.loginapp.onAppActiveTick.id)
        assert handler_cls is not None
